...
import { CAREERS, BACHELORS, bachelorMatches, type Career } from "@/lib/careers";

export const Route = createFileRoute("/bachelors")({
  head: () => ({
    meta: [
      { title: "Bachelors — PoliMiPathways" },
      { name: "description", content: "Choose your bachelor's degree and explore the careers it can lead to." },
    ],
  }),
  component: Bachelors,
});
...
function Bachelors() {
  ...
  const careers = selected
    ? CAREERS.filter((c) => bachelorMatches(c.bachelor, selected))
    : [];
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-24">
      ...
      <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {BACHELORS.filter((b) => b !== "Any Design BSc").map((b) => {
          const count = CAREERS.filter((c) => bachelorMatches(c.bachelor, b)).length;
...
function BachelorCareerCard({ career, onOpen }: { career: Career; onOpen: () => void }) {
  return (
